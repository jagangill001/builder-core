    ...
    if not sources:
        if signals:
            return {
                "level": "medium",
                "signals": signals,
                "explanation": "The user-provided text contains possible manipulation signals, but DuckDuckGo search is not available right now, so Builder Core cannot verify real-world context.",
            }
        return {
            "level": "unknown",
            "signals": [],
            "explanation": "DuckDuckGo search is not available right now, so real manipulation analysis cannot be completed.",
        }
